...
for i in range(0,nimages):
    ...
    #### ***** Unstable **** Changes done by Martin and David ########
    #Reading LHC dataset for plotting
    particle_details = [] #This will the data
    
    # Open the first file and read its content
    with open('small_muon_sample.dat', 'r' ) as file:
        for particles in file: # Read the data obtained line by line 
            no_of_particles = int(particles)  
            ...
            for particle in range(1, no_of_particles): # Loop through the number of particles detected in the decay
                 ...
                 particle_details = particle.split( "   " ) # Split the components assuming they are separated by three spaces
                 energy, x, y, z, charge = particle_details # Unpack the list 
                 print energy, x, y, z, charge
                 
   #### End the **** Unstable**** changes done by Martin and David ########
